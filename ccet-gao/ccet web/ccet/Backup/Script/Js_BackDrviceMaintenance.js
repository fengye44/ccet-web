$(function() {
    $("#idMyTable .btnDel").click(function() {
        ...
        if (_r == true) {
            $.get("Handler1.ashx", { Action: "DeleteDrviceMaintenance", DrviceMaintenanceID: $(this).attr("id") }, function(data, textStatus) {
                if (data == "1") {
                    alert("操作成功！");
                    location.reload();
                }
    ...
    $("#btnFile").uploadify({// 初始化函数
        ...
        'script': "Handler1.ashx?Action=InsertDrviceMaintenance", // 后台处理的请求
...
//            return false;
//        }
    var _DrviceMaintenanceTitle = $("#Text1").val();
    if (_DrviceMaintenanceTitle == "") {
            alert("请输入题目！");
            return false;
        }
        ...
        if (_r == true) {
            $('#btnFile').uploadifySettings('scriptData', { "Action": "InsertDrviceMaintenance",
            "DrviceMaintenanceTitle": _DrviceMaintenanceTitle
            })
